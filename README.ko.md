...
- **요약 마인드맵** — 어떤 요약이든 제목 구조의 인터랙티브 마인드맵으로 전환; 가지를 접기/펼치고 노드의 `[p.N]` 배지를 클릭하면 원문 뷰어의 해당 페이지로 점프
...
설치 후에는 앱이 시작할 때 GitHub 릴리즈에서 새 버전을 확인하고 **메인 화면에서 바로 알려줍니다** — 릴리즈마다 이 페이지를 다시 방문할 필요도, 설정 화면을 찾아 들어갈 필요도 없습니다.

- **확인은 자동, 다운로드는 수동** — 새 버전이 있다는 안내와 함께 **다운로드** 버튼이 표시되고, 실제 전송(약 100MB)은 그 버튼을 누른 뒤에 시작합니다. 종량제·테더링 환경에서 동의 없이 데이터를 쓰지 않습니다
- **전 과정을 메인 화면에서 따라갈 수 있습니다** — 안내가 *사용 가능* → *다운로드 중 %* → *설치 준비됨* 으로 이어지고, 실패하면 사라지는 대신 사유와 **다시 시도** 버튼을 보여줍니다. 안내를 닫으면 조용해지지만, 다운로드가 끝나 설치 준비가 되면 그때는 다시 알려줍니다
- **설치 시점도 사용자 선택** — 다운로드가 끝나면 **재시작하여 설치** 가 표시됩니다. 작업 중이던 요약·Q&A·검색 인덱스는 앱이 닫히기 전에 디스크에 저장되며, 요약·인덱싱이 진행 중이면 설치는 막힙니다(다운로드는 백그라운드라 막지 않습니다)
...
| **커스텀 템플릿** | **설정**에서 나만의 요약 프롬프트를 정의 — 기본 유형과 함께 여기에 표시. 템플릿마다 **단일 패스**(빠름) 또는 **청크+통합**(문서 전체·긴 파일 커버) 처리 방식 선택, 페이지 인용 유지 |

### 3. 결과 확인 및 저장
- 요약이 실시간으로 화면에 표시됩니다
- 요약 헤더의 **텍스트 / 마인드맵** 토글로 전환 — 마인드맵은 요약의 제목을 접을 수 있는 트리로 만들어 탐색하며, 각 노드의 `[p.N]` 배지로 원문 페이지로 점프
- **`.md` 내보내기**·**PDF 내보내기**(헤딩/표/인용이 담긴 서식 PDF)로 파일 저장, **복사** 버튼으로 클립보드에 복사
- 요약 닫기는 비파괴적 — 문서 화면으로 접히고, **요약 보기 / Q&A 계속**으로 Q&A 스레드까지 그대로 다시 엽니다

### 4. Q&A 채팅 (RAG 시맨틱 검색)
- PDF 로드 시 자동으로 **RAG 벡터 인덱스**가 생성됩니다 (헤더에 진행률 → 완료 시 **RAG** 배지 표시). 앱을 켜둔 채 Ollama 임베딩 모델을 설치·삭제해 인덱스가 현재 임베딩 모델과 맞지 않게 되면, 초록 배지 뒤에서 조용히 키워드 검색으로 내려가지 않고 헤더에 그 사실을 표시합니다
...
- **교차 문서 Q&A (컬렉션 모드)** — 문서를 2개 이상 열면 **"여러 문서에 걸쳐 질문"** 토글이 나타납니다. 체크박스로 검색 대상을 고르면 선택한 각 문서의 인덱스에서 질문을 검색해 **재임베딩 없이** 병합합니다. 답변은 출처 문서를 표기하며(예: `[Service Discovery.pdf p.5]`), 인용을 클릭하면 해당 문서로 전환되며 그 페이지로 이동합니다. 임베딩 모델이 다른 문서는 사유와 함께 자동 제외됩니다. 어느 문서인지 하나로 특정할 수 없는 인용(같은 파일명의 문서를 두 개 열어둔 경우)은 엉뚱한 문서로 이동하는 대신 사유와 함께 이동 불가로 표시됩니다.
...
- 텍스트를 추출하지 못한 페이지와 OCR 자체가 실패한 페이지를 알려줍니다 — 표지에만 텍스트 레이어가 있는 혼합 스캔 문서가 나머지 페이지를 빈 채로 두고도 정상 완료처럼 보이지 않습니다(요약·인용·검색이 조용히 그 페이지를 못 보던 문제)
...
- 커스텀 요약 템플릿 — 설정에서 기본 유형과 함께 나만의 이름·프롬프트를 정의; 템플릿마다 단일 패스(빠름) 또는 청크+통합(긴 문서 전체 커버) 처리 방식을 선택, 페이지 인용 유지
...
- 요약 마인드맵 — 텍스트/마인드맵 토글로 요약의 제목 계층을 접을 수 있는 트리로 렌더; 각 노드에 페이지 인용이 `[p.N]` 배지로 붙어 원클릭으로 원문 페이지로 점프(키보드·스크린리더 친화, 외부 의존성 추가 없음)
...
**품질 보증**
- 단위 테스트 1765건 + Playwright E2E + CI 품질 게이트, 릴리즈마다 4-에이전트 병렬 QA 수행
...
| 헤더에 "검색 인덱스가 임베딩 모델과 맞지 않는다"고 표시됨 | 인덱스가 지금 쓰는 것과 다른 임베딩 모델로 만들어진 상태입니다(보통 앱을 켜둔 채 Ollama 임베딩 모델을 설치·삭제한 경우). 문서를 다시 열면 현재 모델로 인덱스가 재생성됩니다 |
| 일부 페이지가 요약·검색에서 빠짐 | 표지에만 텍스트 레이어가 있는 스캔 문서는 나머지 페이지의 텍스트 추출이 비어 있을 수 있습니다. 설정에서 "스캔 PDF OCR"을 켜고 문서를 다시 열어보세요. 이 경우 텍스트를 추출하지 못한 페이지가 안내됩니다 |
| Ollama 주소를 저장할 때 거부됨 | 주소는 스킴을 포함한 전체 URL이어야 합니다(예: `http://localhost:11434`). 저장 시점에 검증합니다 — 이전에는 잘못된 주소도 저장된 것처럼 보이고, 이후 모든 요청이 실패하는데 상태바는 "연결됨"을 표시했습니다 |
...
| 테스트 | Vitest 단위 테스트 1765건/95파일 (renderer·shared 1155 + main 610) + Playwright E2E (CI-결정적 9건) + `tsc --noEmit` 타입 체크 + CI 커버리지 게이트 (81/74/81/84) |
...
- **단위 테스트 1765건 / 95파일** — renderer·shared 1155 + main 610. 메인 프로세스는 electron 모킹 하니스로 IPC 핸들러·OllamaManager·API 키 저장소·ai-service·전체 문서 검색까지 행위 테스트, 렌더러/preload 레이어(컴포넌트 17종 전수 + use-summarize/use-session/pdf-parser/safe-markdown 등 핵심 라이브러리 + preload 브리지)는 happy-dom 으로 행위 테스트
...
- **CI 게이트** — `tsc --noEmit`(strict, e2e 전용 타입체크 프로젝트 포함), 커버리지 임계(81/74/81/84) 강제, lockfile 버전 동기화 검증, 태그 ↔ `package.json` 버전 일치 검증, `npm audit` advisory, Node 22/24 매트릭스 + Windows 유닛 테스트 레그
...
- **4-에이전트 병렬 QA** — 릴리즈마다 전체 코드베이스 QA 라운드 수행, 에이전트별로 다른 축을 담당(최근 변경·동시성·영속화·패키징/CI 등). 50+ 라운드 연속 차단성 0건이며, 지금 라운드가 실제로 발굴하는 것은 **비싸지만 조용한 부류** — 에러 없이 사라지는 데이터, 맞아 보이지만 틀린 답변이다. v0.31.42 에서 고친 두 예: 앞쪽이 목차인 문서에서 **목차 줄이 챕터 번호를 소진**해 뒤따르는 실제 장이 전부 억제됐고(챕터별 요약이 한 장만 남았다), Ollama 가 꺼진 채 문서를 열기만 해도 저장된 검색 인덱스가 삭제돼 다시 열 때 전 문서를 재임베딩해야 했다. 라운드는 설계상 **이전 수정이 만든 회귀**가 드러나는 자리이기도 하다 — 그 라운드에서 나온 결함 중 다섯 건이 직전 두 라운드의 수정으로 소급됐고, 그래서 지금은 모든 수정이 **결함을 먼저 재현하는 테스트**와 함께 들어간다
- 상세 개선·수정 이력: [docs/HISTORY.md](docs/HISTORY.md)
